...
import { ArrowLeft, Loader2, AlertCircle, CheckCircle2, Save, Trash2, Mail } from "lucide-react";
...
export default function EmailAccountSettingsPage() {
    ...
    async function fetchDetail() {
      ...
      try {
        ...
        if (data.error) {
          ...
          setAccounts(prev => prev.map(a => a.id === selectedAccountId ? { ...a, ...data.data } : a));
        }
      } catch {
  ...
  const handleDelete = async () => {
    ...
    const confirmed = window.confirm(`Are you sure you want to delete ${selectedAccount.email}? This action cannot be undone.`);
    if (!confirmed) return;
    ...
    try {
      const res = await fetch(`/api/emailbison/email-accounts/${selectedAccountId}`, { method: "DELETE" });
      const result = await res.json();
    ...
    <div className="min-h-screen bg-black">
      <div className="max-w-6xl mx-auto p-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white">Email Account Settings</h1>
            <p className="text-zinc-400 mt-1">Configure sender names, signatures, and daily limits</p>
          </div>
          <Link href={href("/email-accounts")}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="bg-zinc-900 border-zinc-800 lg:col-span-1">
              <CardHeader className="pb-3">
                <CardTitle className="text-sm font-medium text-zinc-400">Select Email Account</CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                <ScrollArea className="h-[500px]">
                  <div className="space-y-1 p-4 pt-0">
                    {accounts.length === 0 ? (
                      <p className="text-sm text-zinc-500 py-4 text-center">No email accounts found</p>
                    ) : (
                      accounts.map((account) => (
                        <button
                          ...
                          className={cn(
                            ...
                            selectedAccountId === account.id ? "bg-zinc-800 border border-zinc-700" : "hover:bg-zinc-800/50 border border-transparent"
                          )}
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <Badge variant="outline" className={cn("text-[10px] mt-0.5", getStatusColor(account.status))}>
                                {account.status}
                              </Badge>
                            </div>
                          </div>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-sm font-medium text-zinc-400">
                    {selectedAccount ? "Account Settings" : "Select an account to configure"}
                  </CardTitle>
                  ...
                  <div className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" className={cn("text-[10px]", getStatusColor(selectedAccount.status))}>
                    ...
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-xs text-zinc-500">This name appears in the &quot;From&quot; field of your emails</p>
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="daily_limit" className="text-zinc-300">Maximum Daily Emails (in campaigns)</Label>
                      <Input
                        id="daily_limit"
                        type="number"
                        min={1}
                      ...
                      />
                      <p className="text-xs text-zinc-500">If you have an active campaign, pause and resume for limits to take effect</p>
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="email_signature" className="text-zinc-300">Email Signature</Label>
                      <Textarea
                        id="email_signature"
                      ...
                      />
                      <p className="text-xs text-zinc-500">HTML signature appended to your emails. Keep it short and sweet.</p>
                    </div>

                    <div className="flex items-center justify-between pt-4 border-t border-zinc-800">
                      <Button variant="outline" onClick={handleDelete} disabled={deleteLoading} className="text-red-400 hover:text-red-300 hover:bg-red-500/10 border-red-500/30">
                        {deleteLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Trash2 className="h-4 w-4 mr-2" />}
                        Delete Account
                      </Button>
                      <Button onClick={handleSave} disabled={!hasChanges() || saveState === "saving"} className="bg-white text-black hover:bg-zinc-200">
                        {saveState === "saving" ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Saving...</> : <><Save className="h-4 w-4 mr-2" />Save Changes</>}
                      </Button>
                    </div>
                  </div>
                ) : (
