      ...
      if (res.ok) {
        ...
        if (campaign) {
          setEmailsWithAssignments((prev) =>
            prev.map((e) =>
              e.id === emailId ? { ...e, assignedCampaigns: [...e.assignedCampaigns, campaign] } : e
            )
          );
        }
        ...
        if (email) {
          setCampaigns((prev) =>
            prev.map((c) =>
              c.id === campaignId ? { ...c, assignedEmails: [...c.assignedEmails, email] } : c
            )
          );
        }
      }
    ...
    <div className="min-h-screen bg-black">
      <div className="max-w-6xl mx-auto p-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white">Assign Email Accounts</h1>
          ...
          <Link href={href("/campaigns")} className="flex items-center gap-2 text-sm text-zinc-400 hover:text-white transition-colors">
            ...
            <div className="inline-flex rounded-lg bg-zinc-900 p-1 border border-zinc-800">
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 text-sm rounded-md transition-colors ${
                  viewMode === "by-email" ? "bg-zinc-700 text-white" : "text-zinc-400 hover:text-white"
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 text-sm rounded-md transition-colors ${
                  viewMode === "by-campaign" ? "bg-zinc-700 text-white" : "text-zinc-400 hover:text-white"
                }`}
              >
            ...
            <>
              {filteredEmails.length === 0 ? (
                <p className="text-zinc-500">
                  {showUnassignedOnly ? "All email accounts are assigned to campaigns" : "No email accounts in this workspace"}
                </p>
              ) : (
                        ...
                        <div className="relative">
                          ...
                          {openDropdown === email.id && (
                            <div className="absolute right-0 top-full mt-1 w-48 bg-zinc-800 border border-zinc-700 rounded-lg shadow-xl z-10 overflow-hidden">
                              {campaigns.filter((c) => !email.assignedCampaigns.some((ac) => ac.id === c.id)).length === 0 ? (
                                <p className="px-3 py-2 text-xs text-zinc-500">Already in all campaigns</p>
                              ) : (
                                campaigns
                                  ...
                                  .map((campaign) => (
                                    ...
                                    >
                                      {campaign.name}
                                      {actionLoading === `${email.id}-${campaign.id}` && <Loader2 className="h-3 w-3 animate-spin" />}
                                    </button>
                                  ))
                              )}
                            </div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {email.assignedCampaigns.length === 0 ? (
                          ...
                          email.assignedCampaigns.map((campaign) => (
                            <span key={campaign.id} className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs bg-white/5 border border-white/10 text-zinc-300 rounded-full">
                              ...
                              >
                                {actionLoading === `${email.id}-${campaign.id}-remove` ? <Loader2 className="h-3 w-3 animate-spin" /> : <X className="h-3 w-3" />}
                              </button>
                            </span>
                          ))
                        )}
          ...
          {viewMode === "by-campaign" && (
            <>
              {filteredCampaigns.length === 0 ? (
                <p className="text-zinc-500">
                  {showUnassignedOnly ? "All campaigns have email accounts assigned" : "No campaigns in this workspace"}
                </p>
              ) : (
                <div className="space-y-3">
                  {filteredCampaigns.map((campaign) => (
                    <div key={campaign.id} className="p-4 bg-zinc-900 border border-zinc-800 rounded-lg">
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                        ...
                        <div className="relative">
                          ...
                          {openDropdown === campaign.id + 10000 && (
                            <div className="absolute right-0 top-full mt-1 w-56 bg-zinc-800 border border-zinc-700 rounded-lg shadow-xl z-10 overflow-hidden">
                              {emailAccounts.filter((e) => !campaign.assignedEmails.some((ae) => ae.id === e.id)).length === 0 ? (
                                <p className="px-3 py-2 text-xs text-zinc-500">All emails already assigned</p>
                              ) : (
                                emailAccounts
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {campaign.assignedEmails.length === 0 ? (
                          ...
                          campaign.assignedEmails.map((email) => (
                            <span key={email.id} className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs bg-white/5 border border-white/10 text-zinc-300 rounded-full">
                              <Check className="h-3 w-3 text-emerald-400" />
                              {email.email}
                              <button
                                onClick={() => handleUnassign(email.id, campaign.id)}
