...
import { ClipboardCheck, Loader2, ArrowLeft, Calendar, Building2, User, CheckCircle2 } from 'lucide-react';
...
interface OutcomeFormData {
  ...
  nextStep: 'send_followup' | 'schedule_another' | 'send_proposal' | 'close_won' | 'close_lost' | '';
  notes: string;
...
export default function MeetingOutcomeFormPage() {
      ...
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <div className="max-w-md w-full mx-auto px-6 text-center">
          ...
          <div className="flex flex-col gap-3">
            <Link href={`/admin/workstreams/${slug}/pipeline`}>
        ...
        {loading ? (
          ...
          <div className="space-y-8">
            ...
            <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-5">
              <h2 className="text-lg font-medium mb-3">{context.booking.title}</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <div className="flex items-center gap-2 text-zinc-400">
                  <User className="h-4 w-4" />
                  <span>{context.booking.person_name}</span>
                  <span className="text-zinc-600">({context.booking.person_email})</span>
                </div>
                <div className="flex items-center gap-2 text-zinc-400">
                  <Building2 className="h-4 w-4" />
                  <span>{context.booking.company_name}</span>
                </div>
                <div className="flex items-center gap-2 text-zinc-400 md:col-span-2">
                  <Calendar className="h-4 w-4" />
                  <span>{formatMeetingDateTime(context.booking.start_time)}</span>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData({ ...formData, nextStep: value as OutcomeFormData['nextStep'] })}
                >
